...
export class YandexStorage {
  ...
  static async deleteFile(fileUrl: string): Promise<void> {
    try {
      const key = fileUrl.replace(`https://storage.yandexcloud.net/${BUCKET_NAME}/`, "");
  ...
  static async getSignedUrl(fileUrl: string, expiresIn = 3600): Promise<string> {
    try {
      const key = fileUrl.replace(`https://storage.yandexcloud.net/${BUCKET_NAME}/`, "");
  ...
  static async deletePluginImages(imageUrls: string[]): Promise<void> {
    try {
      const deletePromises = imageUrls.map(url => this.deleteFile(url));
...
export class ImageUtils {
  ...
  static validateImageSize(buffer: Buffer, maxSizeMB: number = 5): boolean {
